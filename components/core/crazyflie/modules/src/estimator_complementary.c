 ...
 *
 * Modified for esp-drone-lite: altitude/position estimation removed,
 * attitude-only complementary filter (2026-08).
 */
...
void estimatorComplementary(state_t *state, sensorData_t *sensorData, control_t *control, const uint32_t tick)
{
  ...
  if (RATE_DO_EXECUTE(ATTITUDE_UPDATE_RATE, tick)) {
    ...
    // esp-drone-lite: positionUpdateVelocity removed (no altitude estimation)
  }

  // esp-drone-lite: position estimation block removed (no TOF/altitude hold)
}

// esp-drone-lite: latestTofMeasurement removed (no TOF sensor)
